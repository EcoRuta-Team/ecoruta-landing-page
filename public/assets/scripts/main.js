...
(() => {
  ...
  if (btnRutaRecomendada) {
    btnRutaRecomendada.addEventListener('click', iniciarRutaEnCurso);
  }

  if (btnRutaAlternativa) {
    btnRutaAlternativa.addEventListener('click', iniciarRutaEnCurso);
  }
...
(() => {
  ...
  // "Seleccionar" buttons go to iniciarRutaEnCurso (already wired above)
  // Wire "Guardar ruta" button in ruta en curso to open save modal
  const btnGuardarRutaEnCurso = document.getElementById('btnGuardarRutaEnCurso');
  if (btnGuardarRutaEnCurso) {
    btnGuardarRutaEnCurso.addEventListener('click', abrirModalGuardar);
  }
